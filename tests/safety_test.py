...
def test_disable_telemetry(tmpdir):
    ...
    requirements_file.write("urllib3==1.24.1")
    assert (
        safety(["--disable-telemetry", str(requirements_file)])
        == EXIT_CODE_VULNERABILITIES_FOUND
    )
...
def test_editable_url_to_tarball_dependency(tmpdir):
    requirements_file = tmpdir.join("requirements.txt")
    requirements_file.write(
        '-e https://files.pythonhosted.org/packages/6a/11/" \
                        114c67b0e3c25c19497fde977538339530d8ffa050d6ec9349793f933faa/lockfile-0.10.2.tar.gz'
    )
    assert safety([str(requirements_file)]) == 0
